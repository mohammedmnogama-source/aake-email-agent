...
def backfill_inbox_history(conn, limit: int = 200) -> int:
    """
    Reads the last `limit` emails from the IMAP inbox and indexes them into ChromaDB.
    This captures historical emails (e.g. from purchase@aqeeqkw.com) that were never
    saved to the SQLite emails table.

    Uses a virtual ID of -(1_000_000 + imap_uid) to avoid clashing with:
      - DB inbox emails (positive IDs: 1, 2, 3...)
      - Sent emails (negative IDs: -uid)
    """
    from imap_tools import A
    from backend.email.fetcher import _strip_html
    from backend.email.imap_client import get_mailbox

    # Always read from the main INBOX (not the watched sub-folder) so we get
    # the full history including purchase@aqeeqkw.com CC emails
    folder = "INBOX"
    ...
    try:
        ...
        for msg in messages:
            ...
            if not body.strip():
                continue
            virtual_id = -(1_000_000 + int(msg.uid))
            try:
                index_email(
                    email_id=virtual_id,
                    body_text=body,
                    subject=msg.subject or "",
                    from_address=str(msg.from_),
                    tone_type="inbox_history",
                    action_taken="",
                )
                count += 1
            except Exception as e:
                print(f"[rag] Failed to index inbox history uid={msg.uid}: {e}")

    except Exception as e:
        print(f"[rag] Inbox history backfill error: {e}")

    if count:
        print(f"[rag] Indexed {count} historical inbox email(s) into ChromaDB")
