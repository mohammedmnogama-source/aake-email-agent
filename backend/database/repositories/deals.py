...
def create_deal(
    ...
    cur = conn.execute(
        """
        INSERT INTO deals (ref_number, customer_name, customer_email, description, source_type, source_raw_text)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (ref, customer_name, customer_email, description, source_type, source_raw_text),
    )
    conn.commit()
